...
object LogSQL {
  ...
  def main(args: Array[String]) {
    ...
    val conf = new SparkConf().setAppName("LogSQL").setMaster("local[*]").set("spark.sql.warehouse.dir", "file:/Users/atekade/tmp/")
   //val ssc = new StreamingContext(conf, Seconds(1))
    val ssc = new StreamingContext("local[*]", "SparkLogSQL", Seconds(1))
    ...
    requests.foreachRDD((rdd, time) => {
      ...
      //wordCountsDataFrame.write.json("/Users/atekade/learning-spark/json-extracts/")
      
    })
    
    // Kick it off
    ssc.checkpoint("/Users/atekade/learning-spark/log-sql/")
    ssc.start()
